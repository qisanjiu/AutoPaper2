...
description: >
  ...
  → M5S03 Introduction & Related Work → M5S04 Methodology
  → M5S05 Experiments & Results → M5S06 Analysis & Discussion
  → M5S07 Abstract & Conclusion → M5S08 Full Draft Assembly & Compilation
...
Phase 2: M5S02 Paper Outline
  ...
  → Conductor advance: M5S02 → M5S03

Phase 3: M5S03 Introduction & Related Work
  → Writing Agent 执行
  ...
  → Conductor advance: M5S03 → M5S04

Phase 4: M5S04 Methodology
  → Writing Agent 执行
  → 问题形式化（与 M2S03 一致）
  → 方法概述 + 核心组件详细描述
...
Phase 5: M5S05 Experiments & Results
  → Writing Agent 执行
...
Phase 6: M5S06 Analysis & Discussion
  → Writing Agent 执行
  → 深入解读（"So what?"）
...
Phase 8: M5S08 Full Draft Assembly & Compilation
  ...
  → Conductor advance: M5S08 → Gate G5
...
- M5S03-M5S08 需遵循 `knowledge/M5/M5S02_paper_outline.md` 中的 Style & Layout Profile 和 Figure Style Profile
...
| M5S02 完成后 | 大纲有页数预算、plotting plan、术语表、Style & Layout Profile、Figure Style Profile、venue 适配 | REVISE → M5S02 |
| M5S03 完成后 | Intro 长度 ≤1.5 页、RW 有对比批判、无泄露 | REVISE → M5S03 |
| M5S04 完成后 | 有伪代码/算法框、与 M2S03-M2S04 一致、符号统一 | REVISE → M5S04 |
| M5S05 完成后 | 所有数值与原始数据一致、表格用 booktabs、图表有引用 | REVISE → M5S05 |
| M5S06 完成后 | 有深入解读（非重复数字）、Limitations 诚实、负面结果可见 | REVISE → M5S06 |
| M5S07 完成后 | Abstract 有具体数值、Conclusion 无新内容、全文数值一致 | REVISE → M5S07 |
| M5S08 完成后 | LaTeX 编译通过、无 orphan cite、无泄露、页数合规 | REVISE → M5S08 |
...
3. **M5S03 完成后**: "Introduction & Related Work 完成。"
4. **M5S04 完成后**: "Methodology 完成。核心组件：[列表]。"
5. **M5S05 完成后**: "Experiments & Results 完成。主结果已交叉验证。"
6. **M5S06 完成后**: "Analysis & Discussion 完成。Limitations 已披露。"
7. **M5S07 完成后**: "Abstract & Conclusion 完成。数值一致性已验证。"
8. **M5S08 完成后**: "论文编译完成。PDF 页数：[N]， orphan cite：[M] 个。"
9. **Gate G5 完成后**: "Gate G5 通过（Logic: X/10, Writing: X/10, Evidence: X/10, Novelty: X/10, Ethics: X/10）。"
10. **M5 完成后**: "M5 论文写作完成。投稿包已生成。"
...
- **Writing Agent 统一负责 M5S02-S08**：论文写作是一个连贯的思维过程，不拆分到不同 Agent（M5S01 除外，由 Analysis Agent 执行审计）。
- **风格蒸馏只做高层抽象**：M5S01/M5S02 可参考相近高水平论文的结构、篇幅、图表和排版规律，但禁止复制原文表达或独特设计。
- **Style & Layout Profile 必须贯穿后续写作**：M5S03-M5S08 必须读取 M5S02 的 Profile，并在与目标 venue 模板冲突时以 venue 模板为准。
...
- **Orphan Cite Gate 必须通过**：M5S08 编译时必须验证所有 `\cite{KEY}` 存在于 `refs.bib`。
