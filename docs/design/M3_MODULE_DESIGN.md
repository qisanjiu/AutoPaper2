...
```
M3S01: Dataset & Environment Review / Setup [Experiment Agent]  → knowledge/M3/M3S01_implementation.md + resource_plan.yaml
M3S02: Baseline Result Review               [Experiment Agent]  → knowledge/M3/M3S02_baseline_lock.md
M3S03: Main Experiment Result Review        [Experiment Agent]  → knowledge/M3/M3S03_main_experiment.md + resource_monitor.csv + runtime watchdog
...
> - 随机种子必须**从本阶段开始固定为 42**（不是等到 M3S03）
...
每个长时间正式 run 必须包含 `experiments/runs/<run_id>/watchdog_checks.jsonl`；出现异常或早停候选时必须包含 `watchdog_alerts.jsonl`，并在 M3S03 正文记录 Agent 决策。全局事件流写入 `experiments/logs/runtime_events.jsonl`。
...
- Watchdog 发现 NaN/Inf、OOM、持续不收敛且无法通过局部修复解决 → FIX/BACKTRACK → M3S03/M3S01/M2S03
...
- 固定 seed=42 下趋势明显但证据不足 → FIX → M3S03（改进配置或补充数据）
...
```
M3S04 发现问题
  ├── 固定 seed=42 下趋势好但证据不足 → FIX → M3S03（改进配置或补充数据，不增加 seed）
...
```
M3S01 hardware probe
  → experiments/configs/resource_plan.yaml
  → M3S03 resolved launch command
...
- 每个正式 run 记录 `resource_monitor.csv`；连续低于阈值时先优化，再将不可优化原因写入 M3S03。
...
| **结果验证** | ⚪ 部分提及 | ❌ 无 | **M3S04 强制固定 seed=42 结果边界审查** |
...
- 但 M3S04 的**范围严格限制**在"验证性分析"（数据质量、固定 seed=42 结果边界、诚实决策），不做"深度解读"（那是 M4 的工作）
