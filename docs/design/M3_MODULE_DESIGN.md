...
- M3 **不做深度解读**——结果深层解释的权力属于 M4 (Analysis)，M3 只负责验证数据质量、固定 seed=42 的结果边界、是否达到成功标准
...
- AutoPaper2 将 baseline 验证严格放在 **M3S03**，主实验放在 **M3S04**，深度分析放在 **M4**
...
> **架构变更说明**: 原设计为 3 Stage (M3S02-S03)。现扩展为 **4 Stage**，将 Baseline Lock 从 M3S02 中拆分为独立 Stage M3S03。理由见 §12 决策 1。

```
M3S02: Dataset & Environment Review / Setup [Experiment Agent]  → knowledge/M3/M3S02_implementation.md + resource_plan.yaml
M3S03: Baseline Result Review               [Experiment Agent]  → knowledge/M3/M3S03_baseline_lock.md
M3S04: Main Experiment Result Review        [Experiment Agent]  → knowledge/M3/M3S04_main_experiment.md + resource_monitor.csv + runtime watchdog
M3S05: Result Validation & Evidence Pack   [Analysis Agent]    → knowledge/M3/M3S05_result_validation.md
...
|---------------|------------------------|---------|
| M3S02 | S11 Code Generation | 新增环境搭建、依赖锁定、可复现清单 |
| M3S03 | — (新增) | 从原 S11/S12 拆出的基线验证阶段；汲取 DeepScientist `baseline` skill 的 comparator-first 原则 |
| M3S04 | S12-S14 合并 | 主实验迭代循环；新增 Evidence Ladder 概念 |
| M3S05 | S15 前半 | 结果验证从"简单报告"升级为"统计检验 + 数据质量审查 + 诚实决策" |

### 2.2 各 Stage 一句话定义

| Stage | 一句话定义 | 成功标准 |
|-------|-----------|---------|
| M3S02 | 审查并锁定数据集、执行环境、依赖和运行配置，同时完成方法代码落地 | 数据集与环境可复现，代码可运行 |
| M3S03 | 审查 baseline 本地运行结果，锁定比较基准并运行 smoke test | baseline contract 完整，smoke test 通过 |
| M3S04 | 审查主实验结果是否超过 baseline，并记录迭代证据 | 主结果可比较，达到 minimum/solid |
| M3S05 | 对实验结果进行统计验证、质量审查，并做出 KEEP/FIX/BACKTRACK 决策 | 决策明确、证据链完整、可追溯 |
...
> - 随机种子必须**从本阶段开始固定为 42**（不是等到 M3S04）

**输出**: `knowledge/M3/M3S02_implementation.md` + `experiments/src/*.py` + `experiments/configs/*.yaml` + `experiments/requirements.lock`
其中必须包含 `experiments/configs/resource_plan.yaml`，记录可见硬件、实际分配、DDP/单卡/CPU 并行/任务并行策略、DataLoader workers、线程环境变量、启动命令模板和利用率阈值。

**质量标准**:
- [ ] 所有 M2S03 中的组件都有对应代码实现
- [ ] 伪代码与代码实现一一对应（或明确记录偏差及理由）
- [ ] 环境依赖已锁定，可在干净环境中重建
- [ ] 资源计划已锁定，多 GPU/多核默认被 DDP 或任务并行利用；未使用资源有原因记录
- [ ] 有 README 说明如何运行代码
- [ ] 随机种子已固定为 42 且记录
- [ ] 代码通过静态检查（无语法错误，可导入）
...
**输入**:
- `knowledge/M3/M3S02_implementation.md` (代码结构)
...
**输出**: `knowledge/M3/M3S03_baseline_lock.md` + `experiments/baselines/*/metric_contract.yaml`
...
- 所有 baseline 指标远低于论文报告 → 检查环境/实现 → 必要时 BACKTRACK → M3S02
...
**输入**:
- `knowledge/M3/M3S03_baseline_lock.md` (已验证的 baseline 和 metric contract)
- `knowledge/M3/M3S01_main_experiment_design.md` (主实验设计：数据集、指标、baseline 参考数值、同条件协议)
- `knowledge/M3/M3S02_implementation.md` (代码实现)

**关键约束**:
> - **Run Contract 锁定**（汲取 DeepScientist `experiment` skill）：在开始主实验前，必须明确记录研究问题、基线参考、数据集/划分、指标键、停止条件、放弃条件、预期输出
> - **Baseline 只读原则**：baseline 代码在本次 Stage 中不得修改（如需修复应在 M3S03 完成）
> - **Evidence Ladder**（汲取 DeepScientist）：明确区分 minimum / solid / maximum 证据目标，不在 minimum 达标前追求 maximum
> - **Runtime Watchdog**：预计超过 2 小时的正式 run 必须周期巡检（默认 4 小时，最长 6 小时），记录 NaN/Inf、不收敛、OOM、异常退出、资源低利用率和早停候选；watchdog 只告警不终止，Agent 决定继续、修复、早停或回溯
...
- Runtime watchdog：每 4 小时检查日志/metric/资源；告警由 Agent 判断，不自动终止
...
- 训练曲线
- 环境快照
- runtime_events.jsonl、watchdog_checks.jsonl、必要时 watchdog_alerts.jsonl 和 Agent 决策日志
```
...
Setup Phase:
    ...
    4. 为长时间 run 启动 runtime watchdog，写入 runtime_events.jsonl

Iteration Loop:
    1. 提出一个实验性修改（基于 M2 设计，或诊断性调整）
    2. git commit -m "exp(iterN): {修改描述}"
    3. 运行完整配置实验，同时写入 resource_monitor 和 watchdog 检查记录
    4. 若 watchdog 告警 NaN/Inf、不收敛、OOM、异常退出或早停候选，Agent 读取日志/曲线/checkpoint 后判断 continue / fix_and_rerun / early_stop / backtrack_request
...
| **maximum** | 全面抛光 | 多数据集、完整曲线、消融预留 | 留给 M4/M5，不在 M3 追求 |

**输出**: `knowledge/M3/M3S04_main_experiment.md` + `experiments/results.tsv` + `experiments/runs/<run_id>/`
每个正式 run 必须包含 `experiments/runs/<run_id>/resource_monitor.csv` 或等价监控日志；低利用率必须触发优化 pass 或记录不可优化原因。
每个长时间正式 run 必须包含 `experiments/runs/<run_id>/watchdog_checks.jsonl`；出现异常或早停候选时必须包含 `watchdog_alerts.jsonl`，并在 M3S04 正文记录 Agent 决策。全局事件流写入 `experiments/logs/runtime_events.jsonl`。

**质量标准**:
- [ ] Run Contract 已锁定并记录
- [ ] 每次迭代都有 git commit 和 results.tsv 记录
- [ ] 主指标与 baseline 的比较基于本地运行结果
- [ ] 随机种子已固定为 42，单次主实验已运行
- [ ] 训练曲线、日志已保存
- [ ] 长时间 run 的 watchdog 巡检、告警和 Agent 决策已保存
- [ ] 环境快照完整
- [ ] 达到 minimum 证据层级；若未达到，明确记录失败原因

**可回溯原因**:
- 方法实现有 bug 导致结果异常 → BACKTRACK → M3S02
- 超参数设置导致训练不稳定 → FIX → M3S04（调整超参）
- Watchdog 发现 NaN/Inf、OOM、持续不收敛且无法通过局部修复解决 → FIX/BACKTRACK → M3S04/M3S02/M2S03
...
**目标**: 对固定 seed=42 的实验结果进行验证、数据质量审查，做出 KEEP/FIX/BACKTRACK 决策，并将证据打包供下游使用。

**输入**:
- `knowledge/M3/M3S04_main_experiment.md` (实验迭代记录)
- `knowledge/M3/M3S03_baseline_lock.md` (baseline metric contract)
...
> - **单次结果边界必须清楚**：未做多 seed 重复实验时，不得声称统计显著或跨 seed 稳定
...
- 可复现性检查：确认配置、日志和命令均记录 seed=42；不要求重复运行

**Step 2: 固定 seed=42 结果验证**
- 记录 seed=42、主指标点估计、与 baseline 的绝对/相对差异
- 不要求 p-value、置信区间、效应量或 mean/std
- 如存在多配置/多 baseline 比较，避免选择性报告
...
| 决策 | 条件 | 后续动作 |
|------|------|---------|
| **KEEP** | 固定 seed=42 下结果支持核心假设，质量无重大问题，声明边界诚实 | 进入 Gate G3 |
| **FIX** | 结果有潜力但存在可修复的问题（如超参未调好、某组件有 bug） | 回溯到 M3S04 或 M3S02 |
...
**可回溯原因**:
- 结果质量有重大问题 → FIX → M3S04 或 M3S02
- 结果不支持核心假设 → BACKTRACK → M2S03 或 M1S04
- 固定 seed=42 下趋势明显但证据不足 → FIX → M3S04（改进配置或补充数据）
...
| **Evidence** | 实验证据是否可信、充分、可比较？固定 seed=42 的声明边界是否恰当？ | 证据达到 solid 层级，seed=42 可追溯 |
...
## 已完成的工作摘要
- 实现了核心方法代码（M3S02）
- 验证了 baseline 并锁定 metric contract（M3S03）
- 执行了主实验迭代循环（M3S04）
- 完成了结果验证与证据打包（M3S05）
...
Phase 1: M3S02 Dataset & Environment Review / Setup
  → Experiment Agent 执行
  → 产出: knowledge/M3/M3S02_implementation.md + 代码文件 + 数据集/环境记录
  → Stage Review: m3_dataset_env_review
  → Review 输出: knowledge/reviews/M3S02_dataset_env_review.md
  → 仅当 Verdict=PASS 时，Conductor 才能推进
  → Conductor advance: M3S02 → M3S03

Phase 2: M3S03 Baseline Result Review
  → Experiment Agent 执行
  → 产出: knowledge/M3/M3S03_baseline_lock.md + baseline metric contracts
  → Stage Review: m3_baseline_result_review
  → Review 输出: knowledge/reviews/M3S03_baseline_result_review.md
  → 仅当 Verdict=PASS 时，Conductor 才能推进
  → Conductor advance: M3S03 → M3S04

Phase 3: M3S04 Main Experiment Result Review
  → Experiment Agent 执行
  → 产出: knowledge/M3/M3S04_main_experiment.md + experiments/results.tsv
  → Stage Review: m3_main_result_review
  → Review 输出: knowledge/reviews/M3S04_main_result_review.md
  → 仅当 Verdict=PASS 时，Conductor 才能推进
  → Conductor advance: M3S04 → M3S05

Phase 4: M3S05 Result Validation & Evidence Packaging
  → Analysis Agent 执行
  → 产出: knowledge/M3/M3S05_result_validation.md
...
- M3S05 的 Stage Review 由独立 `m3_result_validation_review` 执行，Analysis Agent 不兼任 reviewer

**Stage Review Agents**:
- M3S01 reviewer: `docs/AGENTS/critic/m3_main_experiment_design_review/AGENT.md` → `knowledge/reviews/M3S01_main_experiment_design_review.md`
- M3S02 reviewer: `docs/AGENTS/critic/m3_dataset_env_review/AGENT.md` → `knowledge/reviews/M3S02_dataset_env_review.md`
- M3S03 reviewer: `docs/AGENTS/critic/m3_baseline_result_review/AGENT.md` → `knowledge/reviews/M3S03_baseline_result_review.md`
- M3S03 audit: `docs/AGENTS/critic/m3_baseline_lock_audit/AGENT.md` → `knowledge/reviews/M3S03_baseline_lock_audit.md`
- M3S04 reviewer: `docs/AGENTS/critic/m3_main_result_review/AGENT.md` → `knowledge/reviews/M3S04_main_result_review.md`
- M3S05 reviewer: `docs/AGENTS/critic/m3_result_validation_review/AGENT.md` → `knowledge/reviews/M3S05_result_validation_review.md`
...
current:
  module: M3
  stage: M3S02  # → M3S03 → M3S04 → M3S05 → G3
  status: in_progress
...
history:
  - stage: M3S02
    agent: experiment
    completed_at: "..."
    output: "knowledge/M3/M3S02_implementation.md"
```
...
| 节点 | 检查项 | 失败处理 |
|------|--------|---------|
| M3S02 完成后 | **数据集真实可用（绝对禁止仿真/合成数据替代）**、环境配置可审查、代码可运行、环境可重建 | 要求修复，仍失败则 BACKTRACK → M2S03 |
| M3S03 完成后 | baseline 本地结果已审查、至少 1 个 baseline contract 可用、smoke test 通过 | BACKTRACK → M3S02 |
| M3S04 完成后 | results.tsv 完整、主实验结果可比较、达到 minimum 证据 | 要求补充 |
| M3S05 完成后 | 决策明确（KEEP/FIX/BACKTRACK）、统计检验恰当 | 若为 FIX 则回溯，若为 BACKTRACK 则执行 |
...
```
M3S05 发现问题
  ├── 固定 seed=42 下趋势好但证据不足 → FIX → M3S04（改进配置或补充数据，不增加 seed）
  ├── 数据质量问题（泄露、不稳定）→ FIX → M3S04 或 M3S02
  ├── 超参数未调好 → FIX → M3S04
  ├── 实现有 bug → BACKTRACK → M3S02
  ├── 方法实现忠实但效果不达预期 → BACKTRACK → M2S03（重新设计）
  └── 效果不达预期且与假设矛盾 → BACKTRACK → M1S04（修正假设）

M3S04 发现训练异常
  ├── 环境/依赖问题 → FIX → M3S02
  ├── 代码 bug → BACKTRACK → M3S02
  └── 方法设计导致训练不稳定 → BACKTRACK → M2S03

M3S03 发现 baseline 无法复现
  ├── 环境差异 → FIX → M3S02
...
> **重要**：M3 的实验可以在**本地**或**远程服务器**上执行。执行位置和环境配置必须在 M3S02 阶段明确并锁定，确保可复现性。
...
Step 5: 记录环境快照到 M3S02 产出
```

### 10.3 远程执行工作流（SSH 模式）

#### 10.3.0 SSH 模式数据集准备规范（M3S02 前置步骤）
...
#### 10.3.1 SSH 认证初始化流程（M3S02 前置步骤）

当项目创建时配置了 `auth_method: password`，M3S02 必须先完成"密码→密钥"过渡：
...
M3S02-Pre: SSH 认证初始化
  ...
  │                在 M3S02_implementation.md 中标记风险
```

**安全要求**：
- 密钥部署成功后，`execution_env.yaml` 中的 `password` 字段必须清空
- 禁止在 M3S03/M3S04 阶段仍以明文密码进行常规同步
- 如密钥部署失败需继续使用密码，必须在 M3S02 产出中明确记录原因
...
每个项目在创建时会生成 `config/execution_env.yaml`，Experiment Agent 在 M3S02 读取并按此配置执行。
...
M3 的资源策略不是“记录机器配置”，而是“把可见资源转成执行计划并验证实际利用率”。

```
M3S02 hardware probe
  → experiments/configs/resource_plan.yaml
  → M3S04 resolved launch command
  → experiments/runs/<run_id>/resource_monitor.csv
  → low utilization optimize-or-document
```

**默认策略**：
- 多 GPU：优先 DDP / `torchrun --nproc_per_node=<gpu_count>`；若方法或 baseline 不支持，记录替代资源策略，不通过多 seed 重复实验填满多卡。
- 单 GPU + 多核 CPU：自动调 batch size、AMP、DataLoader `num_workers`、`pin_memory`、`persistent_workers`、预取和线程环境变量。
- CPU-only：必须使用多进程/多线程或数据加载并行，除非公平性或内存限制不允许；不做多 seed 重复实验。
- 每个正式 run 记录 `resource_monitor.csv`；连续低于阈值时先优化，再将不可优化原因写入 M3S04。
...
|------|---------------------------|-------------------------|----------------------|
| **Baseline 处理** | 隐式包含在 experiment 中 | 有 Baseline 里程碑 | **M3S03 独立 Baseline Lock 阶段；comparator-first 原则** |
| **Run Contract** | ✅ 显式锁定 | ⚪ 隐含在 Plan 中 | **M3S04 强制 Run Contract 锁定** |
| **Evidence Ladder** | ✅ minimum/solid/maximum | ❌ 无 | **M3S04 强制 Evidence Ladder 评估** |
| **结果验证** | ⚪ 部分提及 | ❌ 无 | **M3S05 强制固定 seed=42 结果边界审查** |
| **失败分类** | ✅ 详细分类 | ❌ 无 | **M3S04 记录失败类型，M3S05 判定** |
| **实验日志** | ⚪ results.tsv | ✅ EXPERIMENT_LOG.md | **results.tsv + M3S04 markdown 报告 + Engineering Findings** |
| **Claim Map** | ❌ 无 | ✅ Claim Map | **M3S04 Run Contract 隐含 claim→evidence 映射；M5S01 Pre-Write Audit 显式整理 claim / evidence / contribution 边界** |
| **Findings 记录** | ❌ 无 | ✅ Research + Engineering | **M3S05 负面结果记录；M4S01 Other Findings 深化** |
| **Artifact 打包** | ✅ `artifact.record_main_experiment` | ❌ 无 | **M3S05 Evidence Artifact 标准化打包** |
| **可比性契约** | ✅ metric contract | ⚪ 部分 | **M3S03 Metric Contract 标准化** |
| **环境管理** | ⚪ bash_exec 本地 | ❌ 无 | **M3S02 隔离环境 + 本地/SSH双模式 + 硬件记录** |
...
原设计：M3S02(代码) → M3S03(实验迭代) → M3S04(结果验证)
...
### 决策 3: 为什么 M3S05 由 Analysis Agent 而非 Experiment Agent 执行？

- 结果验证需要判断固定 seed=42 的证据边界、质量问题和诚实声明，这是 Analysis Agent 的核心能力
- Experiment Agent 的职责是"跑实验"，Analysis Agent 的职责是"判断结果意味着什么"
- 但 M3S05 的**范围严格限制**在"验证性分析"（数据质量、固定 seed=42 结果边界、诚实决策），不做"深度解读"（那是 M4 的工作）
...
- 目的：让下游（M3S04、M3S05、M4、M5）无需重新运行 baseline 就能知道比较基准
...
- M3S04 的 results.tsv 必须记录**所有**迭代尝试，包括失败的
- M3S05 必须设有「负面结果」专节
- 如果 M3S05 决策为 BACKTRACK 因为假设被否定，这不是"失败"，而是"获得了有价值的负面证据"
...
└── AGENT.md                           # 更新：明确 M3S05 的范围限制

docs/AGENTS/critic/evidence/
└── AGENT.md                           # 新增：Gate G3 Evidence Critic

docs/AGENTS/critic/
├── m3_dataset_env_review/AGENT.md      # 新增：M3S02 数据集/环境审查
├── m3_baseline_result_review/AGENT.md  # 新增：M3S03 baseline 结果审查
└── m3_main_result_review/AGENT.md      # 新增：M3S04 主实验结果审查

templates/stage/
├── M3S02_template.md                  # 新增：Dataset & Environment Review / Setup
├── M3S03_template.md                  # 新增：Baseline Result Review
├── M3S04_template.md                  # 新增：Main Experiment Result Review
└── M3S05_template.md                  # 新增：Result Validation & Evidence Packaging
...
spiral/project.py                     # MODULE_STAGES["M3"] 扩展为 4 Stage
                                      # GATE_STAGES["G3"] 改为 "M3S05"
                                      # AGENT_FOR_STAGE 添加 M3S05
...
docs/AGENTS/analysis/AGENT.md          # ⚠️ 存在但需更新（M3S04 相关部分）
