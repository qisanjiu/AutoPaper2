...
3. `M5S02` - `M5S07` 由 Writing Agent 执行，每个 Stage 完成后必须调用对应 M5 stage reviewer
4. `M5S08` 由 Writing Agent + Build Verifier 执行
5. `M5S08` 的 review 由 `m5_final_compilation_review` 执行
6. `G5` 由 Logic + Writing + Evidence + Novelty + Ethics Critics 执行
7. Gate G5 PASS 后，才可进入可选的 Peer Review Simulation 与修订循环
...
- M5S02-M5S07 必须依赖上游 evidence，不得虚构数值或引用
...
- M5S08 只做整合与编译验证，不负责补写新实验结论
- M5 的 build_verifier 只在 `M5S08` 触发，不应提前绑定到 `M5S07`
