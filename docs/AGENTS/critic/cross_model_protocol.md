...
默认策略：
1. `full_regenerate` 是默认值，适用于不确定、跨层级、语义变化大的回溯；它表示重新验证整份产物，不表示清空当前 canonical 文件。
2. `incremental_replay` 仅适用于局部修补且上下游接口未变的场景，可以复用当前 canonical 文件中未受影响的部分作为草稿，但必须重新核对它们依赖的上游原始文件。
3. 当前 canonical 文件必须先读后改，仍正确的 section 应保留；单独的旧 downstream 文件是历史证据，不是新结论的唯一来源。
