...
**M3S01: Dataset & Environment Review / Setup** → `knowledge/M3/M3S01_implementation.md` + 代码文件 + 环境/数据集配置
...
**M3S02: Baseline Result Review** → `knowledge/M3/M3S02_baseline_lock.md`
...
**M3S03: Main Experiment Result Review** → `knowledge/M3/M3S03_main_experiment.md` + `experiments/results.tsv`
...
## 4. Comparator-First Baseline 原则（M3S02）
...
- 在 M3S03 中修改 baseline 代码（如需修复应在 M3S02 完成并记录）

---

## 5. Run Contract 规范（M3S03）
...
### 5.1 Resource Utilization Contract（M3S01/M3S03/M4S03 强制）
...
- 在 M3S03 前生成 `experiments/configs/m3_task_queue.yaml`，在 M4S03 前生成 `experiments/configs/m4_task_queue.yaml`。每个任务至少包含 `task_id`/`run_id`、stage、command、estimated_minutes、dependencies、parallelizable、resource_requirements、fairness_key。
...
**⚠️ 关键原则**：M3S03 **不做保留/回退决策**。所有实验尝试都通过 git commit 保存。最终的 KEEP/DISCARD/FIX 决策由 M3S04 (Analysis Agent) 在深入分析后做出。
...
3. 修复后，后续 `M3S02`、`M3S03` 必须基于新上下文重新执行，旧结果只能作为历史记录。
4. 重新产出 `knowledge/M3/M3S01_implementation.md`。
...
2. 重新锁定 metric contract，更新 `knowledge/M3/M3S02_baseline_lock.md`。
3. 后续 `M3S03` 必须重新比较，不得沿用旧主实验结论。
...
2. 若回溯根因在于实验实现或 baseline，按上述 M3S01-M3S03 规则处理。
...
M3S01 阶段，Experiment Agent 必须按以下优先级尝试获取数据集，**不得跳过任何步骤直接判定"不可获取"**：
...
1. **生成《数据集获取报告》**，写入 `knowledge/M3/M3S01_dataset_pending.md`：
   ```markdown
   # Dataset Pending Report — M3S01
...
4. **等待用户确认**：
   ...
   - 验证通过后，删除 `M3S01_dataset_pending.md`，恢复正常 M3S01 流程

### 9.4 绝对禁止的行为

以下行为在 M3S01 及整个 M3 阶段 **绝对禁止**，违者 Review Agent 应直接判为 BACKTRACK：
...
| 在数据集未确认可用前开始代码实现 | 数据集可用性是 M3S01 的前置条件 |
...
- M3S03 主实验和 M4S03 深度分析实验都必须在同一个 sandbox profile 或兼容 profile 下运行。
...
### 10.4 长任务、权限与等待 Ledger（M3S01 强制）
