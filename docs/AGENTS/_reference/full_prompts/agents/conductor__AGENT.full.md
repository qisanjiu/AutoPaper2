...
1. `M3S02` 由 Experiment Agent 执行
2. `M3S02` 必须同时产出 `experiments/logs/m3s02_longrun_ledger.md`，用于审计下载、上传、远程环境、依赖、checkpoint、smoke run 的长任务等待/恢复/权限状态
3. `M3S02` 的 review 由 `m3_dataset_env_review` 执行
4. 当用户提供多个服务器、多张 GPU 或要求 local+remote 混合计算时，Conductor 只能准备/派发 SSH resource pool（如需），不得自行执行实验；M3S02/M3S04 由 Experiment Agent 使用 `resource_plan.yaml` 与 `m3_task_allocation.yaml` 调度可并行任务
5. `M3S03` 由 Experiment Agent 执行
6. `M3S03` 的 review 由 `m3_baseline_result_review` 执行
7. `M3S04` 由 Experiment Agent 执行
8. `M3S04` 的 review 由 `m3_main_result_review` 执行
9. `M3S05` 由 Analysis Agent 执行
10. `M3S05` 的 review 由 `m3_result_validation_review` 执行
11. `G3` 由 Method Critic + Evidence Critic 执行

**硬约束**：
- 任何 M3 Stage 只有在对应 review 文件存在且 `Verdict: PASS` 时才能推进
- M3S02 缺少 `m3s02_longrun_ledger.md` 或 ledger 中出现因"太大/太慢/需要等"跳过必要数据/checkpoint/上传任务时，不得推进
- M3S04 缺少 `experiments/logs/runtime_events.jsonl`、`watchdog_checks.jsonl` 或告警后的 Agent 决策记录时，不得推进；watchdog 告警本身不是终止条件，必须由 Experiment Agent 读取证据后判断继续、修复、早停或回溯
...
- `rebuild_mode=incremental_replay` 时允许参考当前 canonical 文件和旧 downstream 文件减少冗余，但必须重新核对当前上游输入；`rebuild_mode=full_regenerate` 时也必须先读取当前 canonical 文件并保留仍正确的 section，旧 downstream 文件只能作为历史证据，不得作为新正文模板
...
2. **必须先生成 dispatch packet，再使用 `Agent` 工具创建对应角色的 subagent**来执行重新执行：
   ...
   ```
   将命令输出中的 **Compact subagent launch prompt** 传给 subagent。该 prompt 只包含 dispatch packet 路径、任务类型、stage/gate 和输出路径。主 Agent 不得把父对话上下文、上游文档全文、stage output 或 review output 粘贴给 subagent，也不得直接写掉这些输出文件。
...
4. **必须使用 `Agent` 工具创建对应角色的 subagent**来执行重新执行，例如：
   ...
   - M3S05/M4S01/M4S02/M4S04 → Analysis Agent subagent
...
5. 传给 subagent 的 prompt 必须遵守 packet-path-only 交接：
   - 只传 compact launch prompt / dispatch packet 路径，不传父对话全文
   - subagent 必须先读取 dispatch packet，再读取 packet 中的 `agent_md`、`md_protocol`、`input_docs`、`backtrack_advice` 和 `output_path`
   - 所有上游输入只传**文件路径**，不转述内容
   - subagent 必须遵守 packet 中的 `context_policy`，对大文件/目录先索引或按需读取，不把日志、论文、PDF 全文塞入上下文
   - 回溯重新执行时，packet 中的 `backtrack_advice` 是修正真源；subagent 必须读取当前 canonical `output_path`，保留重新验证后仍正确的内容；旧 downstream 文件只能作为历史证据

### 10.3 Rebuild Mode 的传递与执行

- `rebuild_mode=full_regenerate`：subagent 必须重新验证目标 stage 的整份产物，但仍应在当前 canonical 文件上保留仍正确的 section；旧 downstream 文件视为历史审计记录，不得复制粘贴作为新正文模板。
- `rebuild_mode=incremental_replay`：subagent 可引用当前 canonical 文件和旧 downstream 文件减少重复劳动，但所有保留内容必须重新对照当前上游输入验证。
...
1. **`backtrack` 命令支持 `--review-file`**：
   ```bash
   python scripts/state_manager.py backtrack M3S04 M3S02 "bug found" \
     --review-file knowledge/reviews/human_m3s04_review.md
   ```
...
2. **`backtrack` 命令支持显式结构化标志**：
   ```bash
   python scripts/state_manager.py backtrack M3S04 M3S02 "baseline mismatch" \
