...
_BLOCKER_PATTERNS: tuple[re.Pattern[str], ...] = tuple(
    ...
    for pattern in (
        ...
        r"\b(insufficient\s+training|external\s+baseline\s+match\s*(?:fail|failed|no))\b",
        ...
        r"(待定|暂定|占位|待补|待处理|未完成|未解决|失败|缺失|不可用|无法下载|下载失败|未下载|未验证|仍在运行|等待人工|需要人工|需要用户|不合格|不可比较|未实现|未运行|训练不足|仅作参考|只作参考|泄露|旁路|绕过信道|推迟到M4|留到M4)",
