...
- baseline comparability / eligibility check 为 fail/❌，或任一 primary baseline 标为 `ineligible` / `non_comparable`；
- 主指标或声明指标 `not implemented`、`proxy only`、`not run`、或未绑定 `metric_protocol_id`；
...
- formal result row 超出 `normal_reference_range` 但没有 anomaly_triage / evidence path / 保留或排除理由；
- implementation shortcut、metric/data/label leakage、protocol mismatch、invalid/diagnostic result row 仍被当作 main evidence。
...
- implementation shortcut / metric-data-label leakage / protocol mismatch → `target_stage` 应指向拥有错误假设的 stage（通常 M3S02 / M3S03 / M3S04），`rerun_scope` 必须包含受影响的 downstream stages；
- markdown-only reviewer advice 不得扩写成具体代码补丁；`required_fix` 应写 inspect/verify/repair/evidence/rerun；
- 已产出的无效或诊断 row 必须标入 `experiments/tables/results_invalid.tsv`，不能作为 limitation 交给 M4。
