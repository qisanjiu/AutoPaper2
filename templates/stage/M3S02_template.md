...
如果任务仍在运行或需要用户凭证/许可证/配额/存储批准，M3S02 必须进入阻塞状态，review 必须给出 `HALT` 或 non-PASS verdict；不得写 `PASS` 后继续推进。阻塞记录必须说明已尝试的获取方式、session/PID/log path、下一次轮询时间、恢复命令和等待用户批准的具体原因。
