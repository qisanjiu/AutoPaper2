...
> **版本**: 0.1.0  
> **核心理念**: 6 模块螺旋式推进，deepResearch 启发的记忆与迭代系统，三层 Critic 质量把关机制  
...
4. [核心机制](#核心机制)
   ...
   - [Orchestrator 身份持久化](#orchestrator-身份持久化)
   - [Gate 门控评审](#gate-门控评审)
...
| 调研记忆 | 无 | **持久化 `survey_memory.yaml`** |
| 迭代搜索 | 单次搜索 | **3-Round 搜索->验证->迭代循环** |
| 来源追踪 | 内嵌 Markdown | **结构化 `M1_source_log.yaml`** |
| 覆盖率审查 | 未检查 | **Gate G1 Coverage Critic** |
| 回溯机制 | 简单重试 | **完整 Backtrack + Spiral Count** |
| 公共文献库 | 无 | **SQLite + FTS 跨项目复用** |
| 投稿模块 | 无 | **M6 外部审稿与修订循环** |
| 编排者防护 | 无 | **运行时边界强制检查** |
...
- **编排者身份持久化**：主 Agent 身份通过状态文件和运行时守卫持久化锁定，对话中断后可恢复
...
主 Agent 的编排者身份 **不能仅依赖 LLM 工作记忆**。当对话暂停并恢复时（如用户说 "继续"），主 Agent 必须通过以下持久化机制重新确立编排者角色：

1. **状态文件锁** (`pipeline_state.yaml`)：
   - `session.orchestrator_lock`: 主 Agent 活跃时始终为 `true`
   - `session.last_agent_mode`: 会话开始时设为 `"orchestrator"`
   - 通过 `PipelineState.assert_orchestrator_mode()` 读取

2. **运行时边界守卫** (`scripts/orchestrator_guard.py`)：
   - 编排者在任何写入操作前必须运行：
     ```bash
     python scripts/orchestrator_guard.py <project_root> <target_path>
     ```
   - 退出码 1 表示 **禁止** -- 该路径属于 Subagent
   - 该脚本通用，适用于 Claude Code、KimiCode、Codex 等

3. **Skill Manifest 注入**：
   - 每个编排者和模块 Skill 以 `ORCHESTRATOR MANIFEST` 块开头
   - 该块位于 frontmatter 后的首段内容，确保在上下文压缩后仍能保留

4. **上下文恢复协议**（暂停后强制执行）：
   ```bash
   python scripts/state_manager.py status
   python scripts/state_manager.py dispatch next --write
   # 将生成的 packet 委托给 Subagent -- 不要自己执行内容
   ```
...
补全配置后回复 **"已填写"** 或 **"done"**，Agent 验证通过后解除阻塞，允许进入 M1S01。
...
项目创建在 `projects/{sanitized_name}-{YYYYMMDD-HHMMSS}/`，自动初始化：
- `state/pipeline_state.yaml` -- 全局状态
- `state/survey_memory.yaml` -- 调研记忆
- `state/research_brief.yaml` -- 项目入口清单
- `state/onboarding_checklist.md` -- 入项检查清单
- `knowledge/M1/` ~ `M6/` -- 模块知识目录
- `drafts/M1S01/` ~ `M6S06/` -- Stage 草稿目录
- `artifacts/` -- 最终产出（LaTeX/PDF）
...
| 探测项 | 自动填写 | 需手动确认 |
|--------|---------|-----------|
| Python / CUDA / GPU | 是 | 确认即可 |
| 环境管理工具（conda/uv/venv） | 是 | 确认即可 |
| PyTorch / TensorFlow 版本 | 是 | 确认即可 |
| SSH 主机/用户/密钥 | 否 | 必须手动填写 |
| 作者信息 | 否 | 必须手动填写 |
...
|   |-- dispatch.py              # Subagent 调度包生成
|   |-- verdict_parser.py        # Stage review 裁决解析器
|   |-- revision_router.py       # 修订路由逻辑
|   |-- project_entry.py         # 项目入口与入项配置
|   |-- public_db/               # 公共文献数据库
|   |   |-- config.py
|   |   |-- manager.py
|   |   |-- models.py
...
|   |-- orchestrator_guard.py    # 编排者运行时边界守卫
|   |-- env_probe.py             # 环境自动探测
|   |-- paperreview_uploader.py  # paperreview.ai 自动投稿
|   |-- email_monitor.py         # IMAP 邮件监控（接收审稿意见）
|   |-- generate_image.py        # 图片生成辅助
|   |-- agent_dispatch.py        # Agent 调度工具
|   |-- test_health_check.py     # 测试健康检查
|   |-- agent_consistency_check.py  # Agent 一致性检查
|-- utils/
|   |-- file_guard.py            # 命名与位置校验
|   |-- stage_gate.py            # Stage 质量检查
|   |-- gate_rubric.py           # Gate 评分标准加载
...
|   |-- 07_MD_PROTOCOL.md        # Markdown 输出协议
...
|   |-- execution_env.yaml       # 执行环境配置（gitignored）
|   |-- figure_style_profiles.yaml
|   |-- author_info.yaml         # 作者信息（gitignored）
